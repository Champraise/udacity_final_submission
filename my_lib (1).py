...
def build_model(arch, hidden_units, dropout, learning_rate, device):
    ...
    if arch == "vgg16":
        ...
        print("Im sorry but {} is not a valid model. You use vgg16, densenet121, or alexnet?".format(arch))

    # Freeze parameters so we don't backpropagate through them
    for param in model.parameters():
        param.requires_grad = False

    # Total Number of inputs (224*224) = 50176
    # 50% of the input features ie. 25088 is used for traning the model
    input_features = 25088

    # hidden units
    hidden_units = 1000

    # Dropout module with 0.03 drop probability
    dropout = 0.03

    # Calculate number of desired output
    output_layer = len(cat_to_name)

    # New Feedforward network defined for classifier with dropout and ReLU activation
...
def save_checkpoint(arch, epochs, model, optimizer, train_datasets, save_dir):
...
def load_model(checkpoint_pth):
    ...
    # Use available pytorch model
    # Default archtecture vgg1
    
    model = models.vgg16(pretrained=True)
    if checkpoint['architure'] == 'densenet121':
        model = models.densenet121(pretrained=True)
    elif checkpoint['architure'] == 'alexnet':
        model = models.alexnet(pretrained = True)
